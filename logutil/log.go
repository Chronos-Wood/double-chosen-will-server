...
const (
	...
	defaultMaxLogFileSize = 400 * 1024 //Byte
	defaultLogFormat      = "text"
	defaultLogLevel       = log.InfoLevel
	defaultFileName       = "server"
	defaultMaxDayRemain   = 30
	defaulMaxLogFileRemained = 120
)

//FileLogConfig file log related configuration
type FileLogConfig struct {
	// log file name
	FileName string `json:"fileName"`
	// maximum file size, in Byte
	MaxSize uint32 `json:"maxSize"`
	// maxmum days during which we remain log files
	MaxDays uint16 `json:"maxDay"`
	// maxmum log file amount remained
	MaxBackups uint32 `json:"maxBackup"`
}

//LogConfig Log related configuration
type LogConfig struct {
	Level            string        `json:"level"`
	Format           string        `json:"format"`
	DisableTimestamp bool          `json:"disableTimestamp"`
	File             FileLogConfig `json:"fileLogConfig"`
}
...
func initFileLog(cfg *FileLogConfig, logger *log.Logger) error {
	...
	if cfg.MaxDays == 0 {
		cfg.MaxDays = defaultMaxDayRemain
	}
	if cfg.MaxSize == 0 {
		cfg.MaxSize = defaultMaxLogFileSize
	}
	if cfg.MaxBackups == 0{
		cfg.MaxBackups = defaulMaxLogFileRemained
	}
...
// InitLogger initializes server's logger.
